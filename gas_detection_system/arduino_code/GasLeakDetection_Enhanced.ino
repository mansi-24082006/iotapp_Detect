...
void fetchCloudSettingsAndCommands() {
  ...
  // Manual control toggles (can be used while system is latched)
  String manualFanBody;
  if (firebaseGet("/controls/fan", manualFanBody)) {
    if (manualFanBody != "null") {
      fanLatchedOn = parseFlexibleBool(manualFanBody, fanLatchedOn);
    }
  }

  String manualServoBody;
  if (firebaseGet("/controls/servo", manualServoBody)) {
    if (manualServoBody != "null") {
      servoLatchedClosed =
          parseFlexibleBool(manualServoBody, servoLatchedClosed);
    }
  }

  // Global reset from mobile UI
  String resetBody;
  bool reset = false;
  if (firebaseGet("/controls/reset", resetBody)) {
    reset = parseFlexibleBool(resetBody, false);
  }

  // Safety interlock: do not allow reset/manual open while gas is currently
  // detected.
  if (!gasDetected) {
    if (reset) {
      ...
      // Mirror state back to manual control tracks
...
void publishTelemetry() {
  ...
  doc["status"] = gasDetected
                      ? "GAS_DETECTED"
                      : (gasEventLatched ? "WAITING_MANUAL_RESET" : "NORMAL");
